...
#     pred = z_inverse(y_, x_stats['mean'], x_stats['std'])
#
#     error = y - pred
#
#     result = {}
#     for i in [2, 5, 8, 11]:
#         te = perfMetrics(error[:, i, :, :], y[:, i, :, :])
#         print('Time Step %i: MAPE %.3f; MAE  %.3f; RMSE %.3f.' % (i + 1, te[0], te[1], te[2]))
#         result.update({str(i+1): te})
#     return result
